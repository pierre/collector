...
def generate_netflow_events_from_dumps(directory):
    ...
    for file in glob.glob(os.path.join(directory, 'nfcapd.????????????')):
        ...
        nfdump = subprocess.Popen(['nfdump', '-q', '-r', file, '-o', 'csv'], stdout=subprocess.PIPE)
        csvOutput = nfdump.communicate()[0]

        events.extend(parse_data(csvOutput))
        try:
            os.remove(file)
            log.debug('Deleted file: %s', file)
        except os.error, err:
            log.error('Unable to delete file [%s]: %s', file, err)
...
def parse_data(data):
    ...
    for line in data.split('\n'):
        payload = line.split(',')
        if len(payload) > 1:
            events.append(create_event(payload))
    log.info('Created %d events', len(events))
    return events


def create_event(netflowPayload):
    ...
    try:
        ...
        log.info('Ignoring bad line: %s', netflowPayload)
        return None
