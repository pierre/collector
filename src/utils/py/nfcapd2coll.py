...
class NetflowDumpSender(threading.Thread):
    ...
    def __parse_data(self, data):
        ...
        for line in data.split('\n'):
            payload = line.split(',')
            if len(payload) > 1:
                self.events.append(create_event(payload))
    ...
    def __create_event(self, netflowPayload):
        ...
        try:
            ...
            log.info('Ignoring bad line from file %s: %s', this.infile, netflowPayload)
            return None
    ...
    def __send_events(self):
        ...
        transport.open()
        log.info('Sending %d messages from file %s to %s:%d', len(events), self.infile, self.scribeHost, self.scribePort)
...
def generate_netflow_events_from_dumps(directory, scribeHost, scribePort):
    ...
    for file in glob.glob(os.path.join(directory, 'nfcapd.????????????')):
        ...
        sender = NetflowDumpSender(file, scribeHost, scribePort)
